...
import itertools
import string
from music21 import converter, note, stream, scale, pitch
...
def segment_into_words(s, english_words, all_names, max_word_length):
    """
    Segments a string into valid words or names using dynamic programming.
    ...
    Returns:
        list or None: List of words if segmentation is possible, else None.
    """
    n = len(s)
    dp = [None] * (n + 1)
    dp[0] = []
...
def main():
    ...
    if unique_results:
        ...
        print("Best Phrases with the Fewest Number of Words:")
